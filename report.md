...
The mapping is direct: `data.py` owns the byte tokenizer and train-validation split, `model.py` owns the GPT-style neural network, `train.py` owns optimization and checkpoint saving, `generate.py` owns single-prompt sampling, and `evaluation/` owns plotting, metrics, prompt batches, and qualitative comparison artifacts.
...
Lower values are better for both metrics. Model B should generally have lower cross-entropy and perplexity than Model A, although the exact result depends on training duration, random seed, hardware, and whether either model overfits. The metrics are saved in `evaluation/metrics.csv` so they can be included in the final README, tables, and presentation materials.

## Qualitative Generation Comparison Against Gemini Flash

The local models generate continuations from the prompts in `evaluation/prompts.txt`. Because they are byte-level mini-models, their outputs are expected to be imperfect. Model A may produce short readable fragments mixed with broken words, unstable punctuation, and repetition. Model B should usually produce more convincing local texture and better line structure, but it can still lose coherence or repeat patterns.
